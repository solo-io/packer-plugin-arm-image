...
func TestUsb(t *testing.T) {
	verifyDev(t, UsbLsBlk, UsbUdev)
}

func TestSdCard(t *testing.T) {
	verifyDev(t, SdCardLsBlk, SdCardUdev)
}

func verifyDev(t *testing.T, blkdata, udevdata string) {

	devices, err := ParseLSBLKDevices([]byte(blkdata))
	if err != nil {
		t.FailNow()
	}
	...
	udev, err := ParseUdevAdm([]byte(udevdata))
	if err != nil {
		t.FailNow()
	}
	device.udevInfo = udev

	if len(device.Children) != 2 {
		t.Errorf("num of children is not 2")
	}
	mntTable, err := ParseMountTable(nil)
	if err != nil {
		t.FailNow()
	}

	rdev, err := GetDevice(&device, mntTable)
	if err != nil {
		t.FailNow()
	}

	if rdev.ReadOnly != false {
		t.Errorf("device is readonly")
	}
	if rdev.Removable != true {
		t.Errorf("device is not removable")
	}
}
